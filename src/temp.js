...
import DisplayResult from '../utils/DisplayResult';    // Import the new component
import { GlobalStyle, ResultArea, Error, HomeButton, Button } from '../App.styled.js';  // Update path
...
const CostCalculatorApp = () => {
  ...
  const fetchAccessToken = useCallback(async () => {
    if (!creds.clientId || !creds.clientSecret || !creds.gatewayUrl || !creds.refreshToken) {
  ...
  const fetchCountries = useCallback(async () => {
    ...
    if (token) {
      try {
        ...
        setError(`Error fetching countries: ${error.response?.data?.message || error.message}`);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    if (creds.gatewayUrl && creds.clientId && creds.clientSecret && creds.refreshToken) {
      fetchCountries();
    } else {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (values) => {
    ...
    const { age, annual_gross_salary, employment_term, country, title, include_benefits, include_cost_breakdowns, regional_to_employer_exchange_rate } = values;
      ...
      employments: [
        {
          ...
          employment_term,
          region_slug,
          regional_to_employer_exchange_rate: regional_to_employer_exchange_rate.toString(),  // Ensure it's a string
          title,
        },
      ],
      include_benefits,
  ...
  const handleDownloadPDF = async (values) => {
    console.log("Download PDF with values:", values);
    const { age, annual_gross_salary, employment_term, country, title, include_benefits, include_cost_breakdowns, regional_to_employer_exchange_rate } = values;
    ...
    try {
      const response = await axios.post(
        `${creds.gatewayUrl}/v1/cost-calculator/estimation-pdf`,
        payload,
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      link.href = url;
      link.setAttribute('download', 'cost_estimation.pdf'); // The file name
  ...
  const formFields = countries.length > 0 ? [
    {
      name: 'country',
      label: 'Country',
      type: 'select',
    ...
    <>
      ...
      <div className="App">
        ...
        {formFields.length > 0 ? (
          ...
          <div>{isLoading ? 'Loading...' : 'Please fill out the credentials form to proceed.'}</div>
        )}
